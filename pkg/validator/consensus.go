...
func (n *Node) handleBlockProposal(prop shadownet.BlockProposalPayload) {
	...
	pipeline := tx.NewPipeline(tx.Deps{Store: txn, StateTree: n.tree, ZK: n.zkSys, ZKTree: n.zkTree, ZKRoots: n.zkRoots, Vault: n.vlt, Silent: n.silentMon, Oracle: n.oracleQuorum, Governance: n.governanceParams, Epoch: types.EpochNumber(prop.Epoch), Now: func() time.Time { return now }})
...
func (n *Node) handleBlockAnnounce(ann shadownet.BlockAnnouncePayload) {
	...
	pipeline := tx.NewPipeline(tx.Deps{Store: txn, StateTree: n.tree, ZK: n.zkSys, ZKTree: n.zkTree, ZKRoots: n.zkRoots, Vault: n.vlt, Silent: n.silentMon, Oracle: n.oracleQuorum, Governance: n.governanceParams, Epoch: types.EpochNumber(ann.Block.Epoch), Now: func() time.Time { return now }})
	results := pipeline.ProcessBatch(entries)
	for _, res := range results {
		if res.Error != nil {
