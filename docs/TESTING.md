...
| Pipeline | Happy path 5 stages; fail at each stage rolls back nullifier; concurrent 1s batches. | `tx.TestPipelineHappyPathTransfer`, `tx.TestPipelineAtomicityReleasesNullifierOnLaterStageFailure`, `tx.TestPipelineExpiredTxRejected` (stage 2), `tx.TestPipelineTamperedSignatureRejected`, `tx.TestPipelineTamperedTxIDRejected`, `tx.TestPipelineMissingSignerPubKeyRejected`, `tx.TestPipelineBankDepositBufferMismatchRejected` (stage 4), `tx.TestPipelineDoubleSpendWithinBatchRejected`, `tx.TestPipelineVoteRecordsBallotCommitment` |
